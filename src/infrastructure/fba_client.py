...
class FBAClientError(Exception):
...
class FBAClient:
    ...
    def _default_audit_context(*, purpose: str, idempotency_key: str | None = None) -> AuditContext:
        trace_id = f"fba-{uuid4()}"
        return AuditContext(
            tenant_id="system",
            actor_type="service",
            actor_id="pms-fba-client",
            scope="tenant",
            purpose=purpose,
            trace_id=trace_id,
    ...
    async def test_connection(self, audit_context: AuditContext | None = None) -> dict[str, Any]:
        try:
            ...
            await self._transport().request(
                ...
                audit_context=self._resolve_audit_context(audit_context, purpose="fba_health_check"),
            )
            return {"status": "ok", "error_code": None, "retryable": False}
        except FileNotFoundError as e:
            raise FBAClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
        except httpx.TimeoutException as e:
            raise FBAClientError(str(e), error_code="timeout", retryable=True)
        except httpx.HTTPStatusError as e:
            status = e.response.status_code
            if status in {401, 403}:
                raise FBAClientError(str(e), error_code="auth_failed", retryable=False)
            if status >= 500:
                raise FBAClientError(str(e), error_code="upstream_5xx", retryable=True)
            raise FBAClientError(str(e), error_code="upstream_4xx", retryable=False)
        except Exception as e:
            raise FBAClientError(str(e), error_code="transport_error", retryable=True)

    async def submit_restock_suggestion(
        self,
        ...
        try:
            ...
            audit = self._resolve_audit_context(
                audit_context,
                purpose="submit_restock_suggestion",
                idempotency_key=(
                    str(enriched.get("idempotency_key"))
                    if enriched.get("idempotency_key") is not None
                    else f"fba-submit-{enriched.get('suggestion_id') or enriched.get('sku') or uuid4()}"
                ),
            )
            ...
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                return {
                    "suggestion_id": enriched.get("suggestion_id") or f"FBA-{uuid4()}",
            ...
            if isinstance(body, dict):
                body.setdefault("owner_domain", "fba")
                body.setdefault("write_object", "recommendation")
                return body
            return {"status": "submitted", "owner_domain": "fba", "write_object": "recommendation"}
        except FileNotFoundError as e:
            raise FBAClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
    ...
    async def get_inventory_levels(
        ...
        try:
            audit = self._resolve_audit_context(audit_context, purpose="get_inventory_levels")
            ...
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                return data if isinstance(data, dict) else {"inventory": data}
            body = await self._transport().request(
                "GET",
            ...
            return body if isinstance(body, dict) else {"inventory": body}
        except FileNotFoundError as e:
            raise FBAClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
    ...
    async def get_fba_shipment_status(
        self,
        shipment_id: str,
        audit_context: AuditContext | None = None,
    ) -> dict[str, Any]:
        try:
            audit = self._resolve_audit_context(audit_context, purpose="get_fba_shipment_status")
            if is_local_artifact_endpoint(self.api_endpoint):
                data = read_json_artifact(self.api_endpoint, self.inbound_path)
                return data if isinstance(data, dict) else {"shipment_id": shipment_id, "status": data}
            body = await self._transport().request(
                "GET",
                self._normalize_resource_path(f"shipments/{shipment_id}"),
                audit_context=audit,
            )
            return body if isinstance(body, dict) else {"shipment_id": shipment_id, "status": body}
        except FileNotFoundError as e:
            raise FBAClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
