...
class PDMClientError(Exception):
...
class PDMClient:
    ...
    def _default_audit_context(*, purpose: str, idempotency_key: str | None = None) -> AuditContext:
        trace_id = f"pdm-{uuid4()}"
        return AuditContext(
            tenant_id="system",
            actor_type="service",
            actor_id="pms-pdm-client",
            scope="tenant",
            purpose=purpose,
            trace_id=trace_id,
        ...
        try:
            ...
            await self._transport().request(
                ...
                audit_context=self._resolve_audit_context(
                    audit_context,
                    purpose="pdm_health_check",
                ),
            )
            return {"status": "ok", "error_code": None, "retryable": False}
        except FileNotFoundError as e:
            raise PDMClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
        except httpx.TimeoutException as e:
            raise PDMClientError(str(e), error_code="timeout", retryable=True)
        except httpx.HTTPStatusError as e:
            status = e.response.status_code
            if status in {401, 403}:
                raise PDMClientError(str(e), error_code="auth_failed", retryable=False)
            if status >= 500:
                raise PDMClientError(str(e), error_code="upstream_5xx", retryable=True)
            raise PDMClientError(str(e), error_code="upstream_4xx", retryable=False)
        except Exception as e:
            raise PDMClientError(str(e), error_code="transport_error", retryable=True)

    async def submit_selection_recommendation(
        self,
        ...
        try:
            ...
            audit = self._resolve_audit_context(
                ...
                idempotency_key=(
                    ...
                    else f"pdm-submit-{enriched.get('recommendation_id') or enriched.get('task_id') or uuid4()}"
                ),
            )
            ...
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                return {
                    "recommendation_id": enriched.get("recommendation_id") or f"REC-{enriched.get('task_id', 'LOCAL')}",
            ...
            if isinstance(body, dict):
                body.setdefault("owner_domain", "pdm")
                body.setdefault("write_object", "recommendation")
                return body
            return {"status": "submitted", "owner_domain": "pdm", "write_object": "recommendation"}
        except FileNotFoundError as e:
            raise PDMClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
