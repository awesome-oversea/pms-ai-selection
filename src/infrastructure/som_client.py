    ...
    async def query_suggestion_status(
        ...
        try:
            audit = self._resolve_audit_context(
                ...
                idempotency_key=f"som-status-{suggestion_id}",
            )
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                for item in items:
                    if isinstance(item, dict) and (item.get("suggestion_id") == suggestion_id or item.get("listing_draft_id") == suggestion_id):
                        return {
                            ...
                            "listing_draft_id": item.get("listing_draft_id"),
                        }
                return {"suggestion_id": suggestion_id, "domain": "som", "status": "not_found"}
            ...
            if isinstance(body, dict):
                body.setdefault("domain", "som")
                body.setdefault("suggestion_id", suggestion_id)
                return body
            return {"suggestion_id": suggestion_id, "domain": "som", "status": "unknown"}
