...
class SCMClient:
    ...
    def _default_audit_context(*, purpose: str, idempotency_key: str | None = None) -> AuditContext:
        trace_id = f"scm-{uuid4()}"
        return AuditContext(
            tenant_id="system",
            actor_type="service",
            actor_id="pms-scm-client",
            scope="tenant",
            purpose=purpose,
            trace_id=trace_id,
        ...
        try:
            ...
            await self._transport().request(
                ...
                audit_context=self._resolve_audit_context(
                    audit_context,
                    purpose="scm_health_check",
                ),
            )
            return {"status": "ok", "error_code": None, "retryable": False}
        except FileNotFoundError as e:
            raise SCMClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
        except httpx.TimeoutException as e:
            raise SCMClientError(str(e), error_code="timeout", retryable=True)
        except httpx.HTTPStatusError as e:
            status = e.response.status_code
            if status in {401, 403}:
                raise SCMClientError(str(e), error_code="auth_failed", retryable=False)
            if status >= 500:
                raise SCMClientError(str(e), error_code="upstream_5xx", retryable=True)
            raise SCMClientError(str(e), error_code="upstream_4xx", retryable=False)
        except Exception as e:
            raise SCMClientError(str(e), error_code="transport_error", retryable=True)
    ...
    async def query_suggestion_status(
        ...
        try:
            audit = self._resolve_audit_context(
                ...
                idempotency_key=f"scm-status-{suggestion_id}",
            )
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                for item in items:
                    if isinstance(item, dict) and (item.get("suggestion_id") == suggestion_id or item.get("purchase_order_id") == suggestion_id):
                        return {
                ...
                return {"suggestion_id": suggestion_id, "domain": "scm", "status": "not_found"}
            ...
            if isinstance(body, dict):
                body.setdefault("domain", "scm")
                body.setdefault("suggestion_id", suggestion_id)
                return body
            return {"suggestion_id": suggestion_id, "domain": "scm", "status": "unknown"}
        ...
        try:
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                payload = await self._transport().request(
                    ...
                    audit_context=self._resolve_audit_context(
                        audit_context,
                        purpose="fetch_supplier_products",
                    ),
                )
    ...
    async def push_product_plan(
        ...
        try:
            audit = self._resolve_audit_context(
                audit_context,
                purpose="push_product_plan",
                idempotency_key=(
                    str(payload.get("idempotency_key"))
                    if payload.get("idempotency_key") is not None
                    else f"scm-plan-{payload.get('product_id') or payload.get('task_id') or uuid4()}"
                ),
            )
    ...
    async def fetch_supplier_quotes(
        self,
        audit_context: AuditContext | None = None,
    ) -> list[dict[str, Any]]:
        return await self.fetch_supplier_products(audit_context=audit_context)

    async def create_purchase_suggestion(
        self,
        payload: dict[str, Any],
        audit_context: AuditContext | None = None,
    ) -> dict[str, Any]:
        try:
            ...
            audit = self._resolve_audit_context(
                ...
                idempotency_key=(
                    ...
                    else f"scm-submit-{enriched.get('purchase_order_id') or enriched.get('task_id') or uuid4()}"
                ),
            )
            ...
            if is_local_artifact_endpoint(self.api_endpoint):
                write_json_artifact(self.api_endpoint, self.outbound_path, enriched)
                return {
                    "purchase_order_id": enriched.get("purchase_order_id") or f"PO-{enriched.get('task_id', 'LOCAL')}",
                    "status": enriched["status"],
                    "supplier_code": enriched.get("supplier_code"),
                    "owner_domain": "scm",
            ...
            if isinstance(body, dict):
                body.setdefault("owner_domain", "scm")
                body.setdefault("write_object", "recommendation")
                return body
            return {"status": "pending_review", "owner_domain": "scm", "write_object": "recommendation"}
        except FileNotFoundError as e:
            raise SCMClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
