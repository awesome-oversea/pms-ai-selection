...
class IAMClient:
    ...
    def _default_audit_context(*, purpose: str, idempotency_key: str | None = None) -> AuditContext:
        trace_id = f"iam-{uuid4()}"
        return AuditContext(
            tenant_id="system",
            actor_type="service",
            actor_id="pms-iam-client",
            scope="tenant",
            purpose=purpose,
            trace_id=trace_id,
    ...
    async def test_connection(self, audit_context: AuditContext | None = None) -> dict[str, Any]:
        try:
            ...
            await self._transport().request(
                ...
                audit_context=self._resolve_audit_context(audit_context, purpose="iam_health_check"),
            )
            return {"status": "ok", "error_code": None, "retryable": False}
        except FileNotFoundError as e:
            raise IAMClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
        except httpx.TimeoutException as e:
            raise IAMClientError(str(e), error_code="timeout", retryable=True)
        except httpx.HTTPStatusError as e:
            status = e.response.status_code
            if status in {401, 403}:
                raise IAMClientError(str(e), error_code="auth_failed", retryable=False)
            if status >= 500:
                raise IAMClientError(str(e), error_code="upstream_5xx", retryable=True)
            raise IAMClientError(str(e), error_code="upstream_4xx", retryable=False)
        except Exception as e:
            raise IAMClientError(str(e), error_code="transport_error", retryable=True)
    ...
    async def get_permission_scope(
        ...
        try:
            audit = self._resolve_audit_context(audit_context, purpose="get_permission_scope")
            if is_local_artifact_endpoint(self.api_endpoint):
                data = read_json_artifact(self.api_endpoint, self.inbound_path)
                return data if isinstance(data, dict) else {"actor_id": actor_id, "scope": data}
            body = await self._transport().request(
                "GET",
                self._normalize_resource_path(f"actors/{actor_id}/scope"),
                audit_context=audit,
                query_params={"scope_type": scope_type},
            )
            return body if isinstance(body, dict) else {"actor_id": actor_id, "scope": body}
        except FileNotFoundError as e:
            raise IAMClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
    ...
    async def submit_scope_request(
        ...
        try:
            ...
            audit = self._resolve_audit_context(
                audit_context,
                purpose="submit_scope_request",
                idempotency_key=(
                    str(enriched.get("idempotency_key"))
                    if enriched.get("idempotency_key") is not None
                    else f"iam-submit-{enriched.get('request_id') or uuid4()}"
                ),
            )
            ...
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                return {
                    "request_id": enriched.get("request_id") or f"IAM-{uuid4()}",
            ...
            if isinstance(body, dict):
                body.setdefault("owner_domain", "iam")
                body.setdefault("write_object", "pending_action")
                return body
            return {"status": "submitted", "owner_domain": "iam", "write_object": "pending_action"}
        except FileNotFoundError as e:
            raise IAMClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
