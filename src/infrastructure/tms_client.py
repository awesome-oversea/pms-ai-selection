...
class TMSClientError(Exception):
...
class TMSClient:
    ...
    def _default_audit_context(*, purpose: str, idempotency_key: str | None = None) -> AuditContext:
        trace_id = f"tms-{uuid4()}"
        return AuditContext(
            tenant_id="system",
            actor_type="service",
            actor_id="pms-tms-client",
            scope="tenant",
            purpose=purpose,
            trace_id=trace_id,
    ...
    async def test_connection(self, audit_context: AuditContext | None = None) -> dict[str, Any]:
        try:
            ...
            await self._transport().request(
                ...
                audit_context=self._resolve_audit_context(audit_context, purpose="tms_health_check"),
            )
            return {"status": "ok", "error_code": None, "retryable": False}
        except FileNotFoundError as e:
            raise TMSClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
        except httpx.TimeoutException as e:
            raise TMSClientError(str(e), error_code="timeout", retryable=True)
        except httpx.HTTPStatusError as e:
            status = e.response.status_code
            if status in {401, 403}:
                raise TMSClientError(str(e), error_code="auth_failed", retryable=False)
            if status >= 500:
                raise TMSClientError(str(e), error_code="upstream_5xx", retryable=True)
            raise TMSClientError(str(e), error_code="upstream_4xx", retryable=False)
        except Exception as e:
            raise TMSClientError(str(e), error_code="transport_error", retryable=True)

    async def submit_logistics_risk_suggestion(
        self,
        ...
        try:
            ...
            audit = self._resolve_audit_context(
                audit_context,
                purpose="submit_logistics_risk_suggestion",
                idempotency_key=(
                    str(enriched.get("idempotency_key"))
                    if enriched.get("idempotency_key") is not None
                    else f"tms-submit-{enriched.get('suggestion_id') or uuid4()}"
                ),
            )
            ...
            if is_local_artifact_endpoint(self.api_endpoint):
                ...
                return {
                    "suggestion_id": enriched.get("suggestion_id") or f"TMS-{uuid4()}",
            ...
            if isinstance(body, dict):
                body.setdefault("owner_domain", "tms")
                body.setdefault("write_object", "recommendation")
                return body
            return {"status": "submitted", "owner_domain": "tms", "write_object": "recommendation"}
        except FileNotFoundError as e:
            raise TMSClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
    ...
    async def get_shipping_rates(
        ...
        try:
            audit = self._resolve_audit_context(audit_context, purpose="get_shipping_rates")
            ...
            if weight_kg is not None:
                params["weight_kg"] = str(weight_kg)
            if is_local_artifact_endpoint(self.api_endpoint):
                data = read_json_artifact(self.api_endpoint, self.inbound_path)
                return data if isinstance(data, dict) else {"rates": data}
            body = await self._transport().request(
                "GET",
            ...
            return body if isinstance(body, dict) else {"rates": body}
        except FileNotFoundError as e:
            raise TMSClientError(str(e), error_code="artifact_missing", retryable=False)
        except ERPClientError as e:
            self._raise_client_error(e)
    ...
    async def get_carrier_performance(
        ...
        try:
            audit = self._resolve_audit_context(audit_context, purpose="get_carrier_performance")
            resource = "carrier-performance"
            params: dict[str, str] = {}
            if carrier_code:
