...
def test_local_selection_close_loop_script_generates_accepted_artifact(tmp_path):
    env = os.environ.copy()
    env.setdefault("SEC_SECRET_KEY", "test-local-selection-close-loop-32chars")

    result = subprocess.run(
        [
            sys.executable,
            "scripts/run_local_selection_close_loop_acceptance.py",
            "--output-root",
            str(tmp_path),
        ],
